...
var EXPAND_TEXT = /(show\s*more|read\s*more|see\s*more|view\s*more|show\s*full|show\s*all|show\s*original|expand|continue\s*reading|আরও|আরো|সম্পূর্ণ|বিস্তারিত|দেখুন|展开|更多|全部|もっと見る|더\s*보기)/i;
...
function injectStyle(){
  try{
    ...
    st.id = '__x_exp_style';
    st.textContent = '[class*="truncat" i], [class*="clamp" i], [class*="collaps" i], [class*="fold" i], [class*="ellipsis" i], [class*="show-more" i], [class*="showmore" i], [class*="line-limit" i], [class*="fade" i], [style*="line-clamp"], [style*="max-height"] { max-height:none !important; height:auto !important; -webkit-line-clamp:unset !important; -webkit-mask-image:none !important; mask-image:none !important; overflow:visible !important; text-overflow:clip !important; }';
    (document.head || document.documentElement).appendChild(st);
  }catch(e){}
}

function forceUnclamp(root){
  var n = 0;
  try{
    var all = (root || document.body).querySelectorAll('div, p, span, section, article, li, pre, td');
    ...
    for(var i = 0; i < lim; i++){
      ...
      try{
        ...
        var clamped = (cs.overflowY === 'hidden' || cs.overflow === 'hidden' ||
...
function expandAllPasses(times, delay, cb){
  ...
  function step(){
    ...
    pass++;
    if(c > 0 && pass < times) setTimeout(step, delay); else cb();
  }
  step();
}

function settle(minWait, cb){
  var t0 = Date.now(), lastLen = -1, stable = 0;
  function step(){
    var len = 0;
    try{ len = (document.body.textContent || '').length; }catch(e){}
    if(len === lastLen) stable++; else { stable = 0; lastLen = len; }
    if((stable >= 3 && Date.now() - t0 >= minWait) || Date.now() - t0 > 12000){
      try{
        requestAnimationFrame(function(){ requestAnimationFrame(function(){ setTimeout(cb, 250); }); });
...
function finalTail(cb){
  var sc = findScroller(), tries = 0, lastH = -1, lastLen = -1, stable = 0;
  function step(){
    tries++;
    try{ sc.scrollTop = sc.scrollHeight; }catch(e){}
    try{ window.scrollTo(0, document.documentElement.scrollHeight); }catch(e){}
    expandOnce();
    forceUnclamp(document.body);
    var h = 0, len = 0;
    try{ h = sc.scrollHeight || 0; }catch(e){}
    try{ len = (document.body.textContent || '').length; }catch(e){}
    if(h === lastH && len === lastLen) stable++; else { stable = 0; lastH = h; lastLen = len; }
    status('শেষের অংশ ' + tries);
    if(stable >= 4 || tries > 45){ cb(); return; }
    setTimeout(step, 350);
  }
  step();
}
...
function shiftMarkers(text, offset){
  ...
  return (text || '').replace(new RegExp(MS + '(\\d+)' + ME, 'g'), function(_, d){ return MS + (parseInt(d, 10) + offset) + ME; });
}
...
function pushMsg(list, role, text, media){
  ...
  if(list.length && list[list.length-1].role === role){
    ...
    last.text += '\n' + shiftMarkers(text, last.media.length);
...
function waitImages(cb){
  ...
  function chk(){
    ...
    if(pending === 0 || Date.now() - t0 > 6000) cb(); else setTimeout(chk, 300);
  }
  chk();
}

try{
  expandAllPasses(4, 300, function(){
    loadHistory(function(){
      sweepDown(function(){
        finalTail(function(){
          waitImages(function(){
            expandAllPasses(3, 300, function(){
              forceUnclamp(document.body);
              status('গোছানো হচ্ছে...');
              settle(1200, function(){ extract(); });
            });
          });
        });
      });
