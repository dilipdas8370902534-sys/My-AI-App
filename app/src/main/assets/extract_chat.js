(function(){
    ...
    var imgBytes = 0;
    var EXPAND_TEXT = /(show\s+more|read\s+more|see\s+more|view\s+more|show\s+full|show\s+all|show\s+original|expand|continue\s+reading|আরও|আরো|সম্পূর্ণ|বিস্তারিত|展开|更多|全部|もっと見る|더\s*보기)/i;
    ...
    function expandOnce(){
        ...
        try {
            ...
            for(i = 0; i < nodes.length; i++){
                ...
                var label = '';
                try {
                    label = (el.innerText || el.value || el.textContent || el.getAttribute('aria-label') || el.getAttribute('title') || '').toLowerCase();
                } catch(e){}
